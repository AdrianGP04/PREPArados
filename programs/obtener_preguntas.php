    ...
    if(!isset($_POST["profesor"])){
        ...
        while ($preguntas = mysqli_fetch_assoc($preguntas_extraido)){
            if($preguntas["PREG_APROB"] == "0"){
                ...
                $modificar = '<button data-toggle="modal" data-target="#myModal" type="button" class="btn btn-link"> Modificar </button>';
                $eliminar = '<button data-toggle="modal" data-target="#myModal" type="button" class="btn btn-link"> Eliminar </button>';
            }
            else{
                $estado = "<span class='approved'> Aprobada </span>";
